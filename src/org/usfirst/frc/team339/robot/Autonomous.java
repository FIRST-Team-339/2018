...
public class Autonomous
{
	...
	public static void init()
	{
		...
		Hardware.autoDrive.setDefaultAcceleration(DRIVE_STRAIGHT_ACCELERATION_TIME);
	} // end Init
	...
	{
		SmartDashboard.putString("Overall Auto state", autoState.toString());
		SmartDashboard.putString("Vision auto state", visionAuto.toString());
		...
		case CHOOSE_PATH:
			...
			case 4:
				...
				if (Hardware.leftAutoSwitch.isOn() == true)
				{
					...
					if (grabData(GameDataType.SWITCH) == Position.LEFT || grabData(GameDataType.SCALE) == Position.LEFT)
				...
				} else
				{
					// if either the switch or scale is on our side, then
					// run the switchOrScalePath
					if (grabData(GameDataType.SWITCH) == Position.RIGHT
							|| grabData(GameDataType.SCALE) == Position.RIGHT)
			...
			default:
				...
				// disable.
				System.out.println("REACHED THE DEFAULT CASE FOR the six position switch)");
				autoState = State.FINISH;
				break;
			}
			break;
	...
	public static boolean autolinePath()
	{
		...
		SmartDashboard.putNumber("Left Encoder", Hardware.leftFrontDriveEncoder.getDistance());
		SmartDashboard.putNumber("Right Encoder", Hardware.rightFrontDriveEncoder.getDistance());

		SmartDashboard.putNumber("Left Motor", Hardware.leftDriveMotor.get());
		SmartDashboard.putNumber("Right Motor", Hardware.rightDriveMotor.get());
		...
		case DRIVE1:
			...
			if (Hardware.autoDrive.driveStraightInches(
					DISTANCE_TO_CROSS_AUTOLINE - Hardware.autoDrive.getBrakeStoppingDistance(), DRIVE_SPEED,
					true) == true)
		...
		default:
			...
			System.out.println("REACHED THE DEFAULT CASE FOR autolinePath()");
		case FINISH:
	...
	public static boolean autoLineScalePath()
	{
		...
		case DRIVE1:
			...
			// the scale
			if (Hardware.autoDrive.driveStraightInches(
					DISTANCE_TO_CROSS_AUTOLINE_AND_GO_TO_SCALE - Hardware.autoDrive.getBrakeStoppingDistance(),
					DRIVE_SPEED, true) == true)
				currentAutolineState = AutolinePathStates.BRAKE1;
			break;
		case BRAKE1:
			// Brake after driving across the line
		...
		default: // prints we reached the default case, then fall through to
					// FINISH
			System.out.println("REACHED THE DEFAULT CASE FOR autoLineScalePath()");
		case FINISH:
			// Stop drive motors and end auto
			Hardware.transmission.stop();
			return true;
	...
	public static boolean leftAutoLineExchangePath()
	{
		...
		SmartDashboard.putString("leftAutolineExchange state : ", leftExchangeAuto.toString());
		...
		case DRIVE_BACK_ACROSS_AUTOLINE:
			...
			if (Hardware.autoDrive.driveStraightInches(
					DISTANCE_BACK_ACROSS_AUTOLINE - Hardware.autoDrive.getBrakeStoppingDistance(), -DRIVE_SPEED,
					true) == true)
			{
		...
		case TURN_90_DEGREES_RIGHT:
			...
			if (Hardware.autoDrive.turnDegrees2Stage(LEFT_SIDE_TURN_TOWARDS_EXCHANGE, TURN_SPEED) == true)
			{
		...
		case DRIVE_TO_EXCHANGE:
			...
			if (Hardware.autoDrive.driveStraightInches(
					LEFT_DISTANCE_TO_EXCHANGE - Hardware.autoDrive.getBrakeStoppingDistance(), DRIVE_SPEED,
					true) == true)
	...
	{
		SmartDashboard.putString("rightAutolineExchange state : ", rightExchangeAuto.toString());
		...
		case DRIVE_ACROSS_AUTOLINE:
			// drives the necessary distance across the autoline then
			// changes the state to DRIVE_BACK_ACROSS_AUTOLINE
			if (Hardware.autoDrive.driveStraightInches(DISTANCE_TO_CROSS_AUTOLINE, DRIVE_SPEED, true) == true)
			{
		...
		case TURN_90_DEGREES_LEFT:
			// turns 90 degrees towards the exchange and sets the state
			// to DRIVE_TO_EXCHANGE
			if (Hardware.autoDrive.turnDegrees2Stage(RIGHT_SIDE_TURN_TOWARDS_EXCHANGE, TURN_SPEED) == true)
			{
		...
		case DRIVE_TO_EXCHANGE:
			// drives distance to the exchange and sets state to DONE
			if (Hardware.autoDrive.driveStraightInches(RIGHT_DISTANCE_TO_EXCHANGE, DRIVE_SPEED, true) == true)
			{
	...
	public static boolean centerSwitchPath()
	{
		...
		case CENTER_INIT:
			...
			// background
			Hardware.cubeManipulator.deployCubeIntake(false);
			Hardware.cubeManipulator.setLiftPosition(SWITCH_LIFT_HEIGHT, FORKLIFT_SPEED);
		...
		case DRIVE_FOUR_INCHES:
			...
			if (Hardware.autoDrive.driveStraightInches(4, AUTO_SPEED_VISION, false) == true)
			{
		...
		case TURN_TOWARDS_LEFT_SIDE:
			...
			if (Hardware.autoDrive.pivotTurnDegrees(-42, AUTO_SPEED_VISION) == true)// Changed
																					// from
																					// 2stageturn
			{
		...
		case TURN_TOWARDS_RIGHT_SIDE:
			...
			if (Hardware.autoDrive.pivotTurnDegrees(38, AUTO_SPEED_VISION) == true)// Changed
																					// from
																					// 2stageTurn
			{
		...
		case DRIVE_STRAIGHT_TO_SWITCH_LEFT:
			...
			if (Hardware.autoDrive.driveStraightInches(DRIVE_NO_CAMERA_LEFT, DRIVE_SPEED, false) == true)
			{
		...
		case DRIVE_STRAIGHT_TO_SWITCH_RIGHT:
			...
			if (Hardware.autoDrive.driveStraightInches(DRIVE_NO_CAMERA_RIGHT, DRIVE_SPEED, false) == true)
			{
				Hardware.axisCamera.saveImage(ImageType.RAW);
				Hardware.axisCamera.saveImage(ImageType.PROCESSED);
				// visionAuto = centerState.DONE;
		...
		case TURN_AGAIN_RIGHT:
			...
			if (Hardware.autoDrive.turnDegrees2Stage(-90, AUTO_SPEED_VISION) == true)
			{
		...
		case LIFT:// DO NOT USE!(too bad I'm using it) it sets the lift position
			...
			Hardware.transmission.stop();
			if (Hardware.cubeManipulator.setLiftPosition(CubeManipulator.SWITCH_HEIGHT, FORKLIFT_SPEED) == true)
			{
				visionAuto = centerState.MAKE_DEPOSIT;
			}
			break;
		...
		default: // prints that we reached the default case, then falls through
					// to DONE
			System.out.println("REACHED THE DEFAULT CASE FOR centerSwitchPath()");
		case DONE:
	...
	public static boolean switchOrScalePath(Position robotPosition)
	{
		...
		case PATH_INIT:
			...
			// onto the next state
			Hardware.cubeManipulator.deployCubeIntake(false);
			Hardware.cubeManipulator.setLiftPosition(CubeManipulator.SWITCH_HEIGHT);
		...
		case DRIVE1:
			...
			if (Hardware.autoDrive.driveStraightInches(
					SWITCH_OR_SCALE_DRIVE_DISTANCE[0] - Hardware.autoDrive.getBrakeStoppingDistance(), DRIVE_SPEED,
					true) == true)
			{
				// turn to switch
		...
		case TURN1:
			...
			if (robotPosition == Position.RIGHT)
			{
				...
				if (Hardware.autoDrive.turnDegrees2Stage(-90, TURN_SPEED) == true)
		...
		case DRIVE2:
			...
			if (Hardware.autoDrive.driveStraightInches(SWITCH_OR_SCALE_DRIVE_DISTANCE[1], FAST_DRIVE_SPEED, true))
		...
		case DRIVE3:
			...
			// going into the opposition zone
				...
				Hardware.autoDrive.driveStraight(SLOW_DRIVE_SPEED, false);
		...
		case TURN_TO_SCALE:
			...
			{
				if (Hardware.autoDrive.turnDegrees2Stage(SWITCH_OR_SCALE_SCALE_ANGLE, TURN_SPEED))
		...
		case BACK_UP_FROM_SCALE:
			...
			Hardware.autoDrive.driveStraight(-SLOW_DRIVE_SPEED, false);
			currentSwitchOrScaleState = SwitchOrScaleStates.RAISE_ARM;
			// DON'T BREAK; , this will continue on into the next state.
		case RAISE_ARM:
			...
			else if (Hardware.cubeManipulator.setLiftPosition(CubeManipulator.SCALE_HEIGHT))
			{
				Hardware.transmission.stop();
		...
		case TURN2:
			...
			if (robotPosition == Position.LEFT)
			{
				if (Hardware.autoDrive.turnDegrees2Stage(90, TURN_SPEED))
					currentSwitchOrScaleState = SwitchOrScaleStates.FINISH;
			} else
			{
				if (Hardware.autoDrive.turnDegrees2Stage(-90, TURN_SPEED))
		...
		default: // prints that we reached the default, then falls through to
					// FINISH
			System.out.println("REACHED THE DEFAULT CASE for switchOrScalePath()");
		case FINISH:
	...
	public static boolean offsetSwitchPath()
	{
		...
		case DRIVE1:
			...
			if (Hardware.autoDrive.driveStraightInches(
					OFFSET_SWITCH_DRIVE_DISTANCES[0] - Hardware.autoDrive.getBrakeStoppingDistance(), DRIVE_SPEED,
					true) == true)
		...
		case DRIVE2L:
			...
			if (Hardware.autoDrive.driveStraightInches(
					OFFSET_SWITCH_DRIVE_DISTANCES[1] - Hardware.autoDrive.getBrakeStoppingDistance(), DRIVE_SPEED,
					true) == true)
		...
		case DRIVE2R:
			...
			if (Hardware.autoDrive.driveStraightInches(
					OFFSET_SWITCH_DRIVE_DISTANCES[2] - Hardware.autoDrive.getBrakeStoppingDistance(), DRIVE_SPEED,
					true) == true)
				currentOffsetSwitchState = OffsetSwitchPath.BRAKE_DRIVE2;
			break;
		case BRAKE_DRIVE2:
		...
		case DRIVE3L:
			...
			if (Hardware.autoDrive.driveStraightInches(
					OFFSET_SWITCH_DRIVE_DISTANCES[4] - Hardware.autoDrive.getBrakeStoppingDistance(), DRIVE_SPEED,
					true) == true)
		...
		case DRIVE3R:
			...
			if (Hardware.autoDrive.driveStraightInches(
					OFFSET_SWITCH_DRIVE_DISTANCES[3] - Hardware.autoDrive.getBrakeStoppingDistance(), DRIVE_SPEED,
					true) == true)
				currentOffsetSwitchState = OffsetSwitchPath.BRAKE_DRIVE3;
			break;
		case BRAKE_DRIVE3:
		...
		default: // prints out a message to let the console know we got here,
			...
			System.out.println("REACHED THE DEFAULT CASE for offsetSwitchPath()");
		case FINISH:
	...
	private static final double SLOW_DRIVE_SPEED = .3;
	...
	{ (int) (AUTO_TESTING_SCALAR * 12), (int) (AUTO_TESTING_SCALAR * 180), (int) (AUTO_TESTING_SCALAR * 44),
			(int) (AUTO_TESTING_SCALAR * 127), (int) (AUTO_TESTING_SCALAR * 117) };
