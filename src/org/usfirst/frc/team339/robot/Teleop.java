...
public static void init()
{
	...
	{
		Hardware.drive.setGearPercentage(0, Hardware.delayPot.get(0, .7));
...
public static void periodic()
{
	...
	if (Hardware.demoModeSwitch.isOn() == false)
	{
		...
		Hardware.cubeManipulator.intakeCube(Hardware.rightOperator.getTrigger(),
				Hardware.rightOperator.getRawButton(2));
		Hardware.cubeManipulator.ejectCube(Hardware.leftOperator.getTrigger(), (Hardware.leftOperator.getRawButton(3)));
	} else
	{
		// We are in DEMO MODE!!!
		Hardware.cubeManipulator.intakeCube(Hardware.rightOperator.getTrigger(), false);
		Hardware.cubeManipulator.ejectCube(false, Hardware.rightOperator.getRawButton(2));
	}
	...
	if (Hardware.demoModeSwitch.isOn() == false)
	{
		...
		if (Hardware.leftOperator.getRawButton(11))
			Hardware.cubeManipulator.deployCubeIntake(Hardware.leftOperator.getRawButton(9));
		else if (Hardware.leftOperator.getRawButton(10))
			Hardware.cubeManipulator.retractCubeIntake(Hardware.leftOperator.getRawButton(9));
	...
	if (Hardware.demoModeSwitch.isOn() == false)
		// We are in COMPETITION MODE!!!
		Hardware.cubeManipulator.moveForkliftWithController(Hardware.rightOperator.getY(),
				Hardware.rightOperator.getRawButton(5), Hardware.climbButton.isOnCheckNow());
	else
		// We are in DEMO MODE!!!
		Hardware.cubeManipulator.moveForkliftWithController(Hardware.rightOperator.getY() * Robot.demoForkliftSpeed,
				false, false);
	...
	if (Hardware.demoModeSwitch.isOn() == false)
	{
		...
		if (Hardware.rightOperator.getRawButton(6) == true)
			Hardware.cubeManipulator.setLiftPosition(CubeManipulator.SCALE_HEIGHT, .6);
		else if (Hardware.rightOperator.getRawButton(7) == true)
			Hardware.cubeManipulator.setLiftPosition(CubeManipulator.SWITCH_HEIGHT, .6);

		if (Hardware.climbButton.isOnCheckNow() == true)
			Hardware.climbingMechanismServo.set(Robot.CLIMB_SERVO_CLIMB_POISITION);
		else
			Hardware.climbingMechanismServo.set(Robot.CLIMB_SERVO_INIT_POSITION);
	}
	...
	if (Hardware.demoModeSwitch.isOn() == false)
		// We are in COMPETITION MODE!!!
		Hardware.axisCamera
				.takeLitPicture(Hardware.leftOperator.getRawButton(6) && Hardware.leftOperator.getRawButton(7));
	...
	if (Hardware.demoModeSwitch.isOn() == false)
		// We are in COMPETITION MODE!!!
		Hardware.drive.shiftGears(Hardware.rightDriver.getRawButton(3), Hardware.leftDriver.getRawButton(3));
	else
	{
		// We are in DEMO MODE!!!
		if (Hardware.leftDriver.getRawButton(7) == true)
			Hardware.drive.setGearPercentage(0, .6);
		else if (Hardware.leftDriver.getRawButton(8) == true)
			Hardware.drive.setGearPercentage(0, Hardware.delayPot.get(0, .6));
		else if (Hardware.leftDriver.getRawButton(9) == true)
			Hardware.drive.setGearPercentage(0, .4);
...
private static void testingDrive()
{
	...
	double speed = Hardware.leftDriver.getThrottle();
	double accel = (Hardware.rightOperator.getZ() + 1);
	double driveStraightConst = (Hardware.rightDriver.getThrottle() + 1) / 2.0;
	double distance = Hardware.leftOperator.getZ() * 120;
	Hardware.drive.setDriveStraightConstant(driveStraightConst);

	SmartDashboard.putNumber("DS Constant", driveStraightConst);
	SmartDashboard.putNumber("Drive Test Speed", speed);
	SmartDashboard.putNumber("Drive Test Acceleration", accel);

	if (Hardware.leftDriver.getRawButton(7))
	{
		testingDrive = true;
		Hardware.drive.driveStraight(speed, accel, true);
	} else
	{
		testingDrive = false;
	}
...
//	if(testingDrive)
//	{
//		if(currentStage == 0 && Hardware.drive.driveStraightInches(distance, speed, accel, true))
//			currentStage++;
//		else if (currentStage == 1 && Hardware.drive.brake(BrakeType.AFTER_DRIVE))
//			currentStage++;
...
public static void printStatements()
{
	if (Hardware.driverStation.isFMSAttached() == false)
	{
		...
		SmartDashboard.putNumber("Cube Motor", Hardware.cubeIntakeMotor.get());
		// System.out.println(
		...
		SmartDashboard.putNumber("6 Pos Switch", Hardware.autoSixPosSwitch.getPosition());

		SmartDashboard.putBoolean("Demo Switch", Hardware.demoModeSwitch.isOn());
		...
		SmartDashboard.putNumber("Left Rear Encoder Inches", Hardware.leftRearDriveEncoder.getDistance());

		// System.out.println("Left Rear Encoder Ticks "
		// + Hardware.leftRearDriveEncoder.get());
		SmartDashboard.putNumber("Left Rear Encoder Ticks", Hardware.leftRearDriveEncoder.get());

		// System.out.println("Right Rear Inches = "
		// + Hardware.rightRearDriveEncoder.getDistance());
		SmartDashboard.putNumber("Right Rear Encoder Inches", Hardware.rightRearDriveEncoder.getDistance());

		// System.out.println("Right Rear Ticks "
		// + Hardware.rightRearDriveEncoder.get());
		SmartDashboard.putNumber("Right Rear Encoder Ticks", Hardware.rightRearDriveEncoder.get());

		// System.out.println("Lift Encoder Inches = "
		// + Hardware.liftingEncoder.getDistance());
		SmartDashboard.putNumber("Lift Encoder Inches", Hardware.liftingEncoder.getDistance());

		// System.out.println(
		// "Lift Encoder Ticks " + Hardware.liftingEncoder.get());
		SmartDashboard.putNumber("Lift Encoder Ticks", Hardware.liftingEncoder.get());

		// System.out.println("Intake Deploy Encoder "
		// + Hardware.intakeDeployEncoder.getDistance());
		SmartDashboard.putNumber("Intake Deploy Encoder", Hardware.intakeDeployEncoder.getDistance());

		// System.out.println("Intake Deploy Encoder Ticks "
		// + Hardware.intakeDeployEncoder.get());
		SmartDashboard.putNumber("Intake Deploy Ticks", Hardware.intakeDeployEncoder.get());
		...
		SmartDashboard.putBoolean("Photo SW", Hardware.cubePhotoSwitch.isOn());
		...
		SmartDashboard.putNumber("AnalogGyro", Hardware.gyroAnalog.getAngle());
		...
		SmartDashboard.putNumber("Front Ultrasonic", Hardware.frontUltraSonic.getDistanceFromNearestBumper());
		//
	...
	SmartDashboard.putBoolean("Too close to scale", Hardware.armIR.isOn());
