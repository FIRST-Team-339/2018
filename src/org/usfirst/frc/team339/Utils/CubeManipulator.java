...
public class CubeManipulator<Button>
{
	 ...
	 */
	public CubeManipulator(SpeedController forkliftMotor, SpeedController intakeMotor, LightSensor intakeSwitch,
			Encoder forkliftEncoder, SpeedController intakeDeploy, Encoder intakeDeployEncoder, Timer timer,
	 ...
	 * 
	 */
	public void moveForkliftWithController(double speed, boolean overrideButton, boolean isClimbing)
	{
		if (isClimbing == true)
		{
		...
		} else
		{
			...
			// motors.
			if ((speed > 0 && forkliftEncoder.getDistance() > FORKLIFT_MAX_HEIGHT)
			...
			/*
			 * if (newCode == true) { // checks to see if the IR reads true, if
			 * so sets the state to // stay at position because we're about to
			 * hit the stupid scale if (armIR.isOn() == true && speed > 0 &&
			 * deployIntakeState != DeployState.FOLDED && deployIntakeState !=
			 * DeployState.FOLD_ARM_DOWN && deployIntakeState !=
			 * DeployState.UNFOLD_ARM_UP) { this.liftState =
			 * ForkliftState.STAY_AT_POSITION; } // if the override is off and
			 * the IR reads false then move based on // joystick else {
			 * this.liftState = ForkliftState.MOVE_JOY; }
			 * 
			 * } else {
			 */
	...
	public boolean deployCubeIntake(boolean override)
	{
		...
		if (deployIntakeState == DeployState.NOT_DEPLOYED || deployIntakeState == DeployState.UNFOLD_ARM_UP)
			;
		{
		...
		{
			if (deployIntakeState == DeployState.DEPLOYED && climbButton.isOnCheckNow() == true)
			{
	...
	{
		if (deployIntakeState != DeployState.STOPPED && deployIntakeState != DeployState.OVERRIDE_DEPLOY
				&& deployIntakeState != DeployState.OVERRIDE_RETRACT)
	 ...
	 */
	public void runIntake(boolean button, boolean pullInOverride, boolean pushOutOverride)
	{
	...
	public boolean pushOutCubeAuto()
	{
		...
		default:
			System.out.println("Error finding state " + pushState + " in CubeManipulator.pushOutCubeAuto()");
		case DONE:
			this.intakeMotor.set(0);
	...
	public boolean scoreScale()
	{
		...
		case MOVE_LIFT:
			if (setLiftPosition(SCALE_HEIGHT, FORKLIFT_DEFAULT_SPEED_UP) == true)
			{
		...
		default:
			System.out.println("Error finding state " + scaleState + " in CubeManipulator.scoreScale()");
	...
	public void forkliftUpdate()
	{
		...
		// deploy
		if (deployIntakeState == DeployState.FOLD_ARM_DOWN || deployIntakeState == DeployState.FOLDED
				|| deployIntakeState == DeployState.UNFOLD_ARM_UP)
		...
		case MOVING_TO_POSITION:
			...
			if ((this.forkliftTargetHeight > FORKLIFT_MAX_HEIGHT)
					...
					|| (this.deployIntakeState == DeployState.DEPLOYED && this.armIR.isOn()))
			{
	...
	public void deployIntakeUpdate()
	{
		...
		case POSITION_45:
			...
			else if (intakeDeployEncoder.get() < DEPLOY_45_POSITION_TICKS - DEPLOY_45_POSITION_DEADBAND)
			{
		...
		case FOLD_ARM_DOWN:
			...
			case RAISE_ARM:
				...
				if (this.getForkliftHeight() > FORKLIFT_DEPLOY_FOLDED_MIN_HEIGHT
						|| setLiftPosition(FORKLIFT_DEPLOY_FOLDED_MIN_HEIGHT) == true)
		...
		case UNFOLD_ARM_UP:
			...
			break;
		default:
			System.out.println("Unkown case found in deployIntakeUpdate(). Stopping deploy.");
		case STOPPED:
	...
	private final double DEPLOY_45_POSITION_TICKS = INTAKE_DEPLOY_TICKS / 2.0;
