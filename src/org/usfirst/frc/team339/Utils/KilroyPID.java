...
public void setEnabled (boolean enabled)
{
    switch (type)
        ...
        case CAN:
            if (enabled == false)
                {
                this.offBoardController.set(ControlMode.PercentOutput,
                        0);
                }
            else
                {
...
public void setSpeed (double velocity)
{
    switch (type)
        {
        case CAN:
            this.offBoardController
                    .configPeakOutputForward(Math.abs(velocity), 0);
            this.offBoardController
                    .configPeakOutputReverse(Math.abs(velocity), 0);
            return;
        case ONBOARD:
            this.onBoardController.setOutputRange(-Math.abs(velocity),
                    Math.abs(velocity));
        default:
            break;
        }
 ...
 */
public void
        tetherCANMotorControllers (BaseMotorController... controller)
{
    switch (type)
        {
        case CAN:
...
public boolean isOnTarget ()
{
    switch (type)
        {
        case CAN:
            return (Math.abs(this.offBoardController
                    .getClosedLoopError(0)) < this.tolerance);
        case ONBOARD:
