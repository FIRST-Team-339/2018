        ...
        GyroBase gyro)
{
    ...
    this.gyroPID[0] = new KilroyPID(
            transmission.getSpeedController(MotorPosition.LEFT_REAR),
            gyro);
    this.gyroPID[1] = new KilroyPID(
            transmission.getSpeedController(MotorPosition.RIGHT_REAR),
            gyro);
}
 ...
 *            units.
 */
public void setPIDTolerance (PIDDriveFunction driveFunction, double p,
        double i, double d, double tolerance)
{
...
public boolean turnDegrees (int degrees, double speed)
{
    ...
    if (turnDegreesInit == true)
        ...
        for (int i = 0; i < encoderPID.length; i++)
            {
            if (i % 2 == 0)
                encoderPID[i].setSetpoint(
                        degreesToEncoderInches(degrees, false), false);
            else
                encoderPID[i].setSetpoint(
                        -degreesToEncoderInches(degrees, false), false);
...
public boolean turnDegreesGyro (int degrees, double speed)
{
    if (turnDegreesGyroInit == true)
        ...
        for (int i = 0; i < this.gyroPID.length; i++)
            ...
            this.gyroPID[i].setPIDF(this.turnGyroPIDTolerance[0],
                    this.turnGyroPIDTolerance[1],
        ...
        boolean isUsingGyro)
{
    if (System.currentTimeMillis()
        ...
        if (isUsingGyro == true)
            ...
            this.driveStraightPID_gyro.getPIDController().setPID(
                    driveStraightGyroPIDTolerance[0],
                    driveStraightGyroPIDTolerance[1],
                    driveStraightGyroPIDTolerance[2]);
            ...
            this.driveStraightPID_enc.getPIDController().setPID(
    ...
    if (isUsingGyro == true)
        super.accelerateTo(speed + driveStraightPIDOutput_gyro,
                speed - driveStraightPIDOutput_gyro, acceleration);
    else
        super.accelerateTo(speed + driveStraightPIDOutput_enc,
                speed - driveStraightPIDOutput_enc, acceleration);
 ...
 */
public boolean driveStraightInches (double speed, double distance,
        double acceleration, boolean isUsingGyro)
{
    ...
    if (driveStraightInchesInit == true)
        {
        resetEncoders();
        driveStraightInchesPID.getPIDController().setPID(
                driveStraightInchesPIDTolerance[0],
                driveStraightInchesPIDTolerance[1],
                driveStraightInchesPIDTolerance[2]);
        driveStraightInchesPID.setAbsoluteTolerance(
                driveStraightInchesPIDTolerance[3]);
    ...
    driveStraight(this.driveStraightInchesSpeed, acceleration,
            isUsingGyro);
    ...
    switch (type)
        ...
        case TURN_ENC:
            ...
            turnPIDTolerance = new double[]
                {
                        encoderPIDTuner.p, encoderPIDTuner.i,
                        encoderPIDTuner.d, encoderPIDTuner.tolerance
                };
            // If enabled, then turn the x degrees. If not, stop.
            if (encoderPIDTuner.enabled)
                {
                if (this.turnDegrees((int) encoderPIDTuner.setpoint,
                        encoderPIDTuner.speed))
                    {
                    stop();
...
{
/**
 * Creates the PIDTuner and sets the name of the shuffleboard widget
 * 
 * @param name
 *            A string describing what the PID controls
 */
...
// Store the PID values
double p, i, d, f, setpoint, speed, tolerance;

// ...And whether it's enabled
boolean enabled;

// SenableBase controls the values being sent to the shuffleboard.
...
public void initSendable (SendableBuilder builder)
{
    ...
    // Adds the p value to the PID shuffleboard tuner
    builder.addDoubleProperty("p", new DoubleSupplier()
    {
    @Override
    ...
    // Adds the i value to the PID shuffleboard tuner
    builder.addDoubleProperty("i", new DoubleSupplier()
    {
    ...
    // Adds the d value to the PID shuffleboard tuner
    builder.addDoubleProperty("d", new DoubleSupplier()
    {
    ...
    // Adds the f value to the PID shuffleboard tuner
    builder.addDoubleProperty("f", new DoubleSupplier()
    {
    ...
    // Adds the setpoint value to the PID shuffleboard tuner
    builder.addDoubleProperty("setpoint", new DoubleSupplier()
    {
    @Override
    public double getAsDouble ()
    {
    ...
    // Adds the enabled switch to the PID shuffleboard tuner
    builder.addBooleanProperty("enabled", new BooleanSupplier()
    {
    ...
    // Adds speed value to the PID shuffleboard tuner
    builder.addDoubleProperty("speed", new DoubleSupplier()
    {
    ...
    // Adds a tolerance value to the PID shuffleboard tuner.
    builder.addDoubleProperty("tolerance", new DoubleSupplier()
    {
