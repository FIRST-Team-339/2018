 ...
 * there is no confusion when tuning PID
 * loops on which PID to tune. Instead the custom PIDTuner class will be used to
 * display / input PID values, as it
 * adds in speed, tolerance and acceleration. (You will need to drag those
 * values out separately from the widget)
 */
 ...
 */
public void setPIDToleranceAccel (PIDDriveFunction driveFunction,
        double p, double i, double d, double tolerance,
        double accel)
{
        ...
        boolean isUsingGyro)
{
    ...
    if (isUsingGyro == true)
        super.accelerateProportionaly(
                speed + driveStraightPIDOutput_gyro,
                speed - driveStraightPIDOutput_gyro, acceleration);
    else
        super.accelerateProportionaly(
                speed + driveStraightPIDOutput_enc,
...
private PIDTuner driveStraightTuner_gyro = new PIDTuner(
        "Drive Straight Gyro");

private PIDTuner driveInchesTuner = new PIDTuner("Drive Inches PID");

private PIDTuner turnDegreesTuner_enc = new PIDTuner(
        "Turn Degrees Encoder");

private PIDTuner turnDegreesTuner_gyro = new PIDTuner(
        "Turn Degrees Gyro");
 ...
 * is useful if you have multiple PID
 * controllers that all need the same tuning at once, so you don't have to
 * change 24 values for a 4 wheel robot.
 * When values are changed in the shuffleboard, the stored values
 * p,i,d,setpoint,tolerance,speed, and enabled will
 * reflect it.
 * 
 * You are able to get a specific widget from this by finding the "LiveWindow"
 * tab on the left, and finding the
 * name of the PID controller (for example, "EncoderPID") and dragging it onto
 * the screen, if there's not
...
public void initSendable (SendableBuilder builder)
{
    ...
    builder.addDoubleProperty("Setpoint", () -> setpoint,
            (arg0) -> setpoint = arg0);
    builder.addDoubleProperty("Speed", () -> speed,
            (arg0) -> speed = arg0);
    builder.addDoubleProperty("Tolerance", () -> tolerance,
            (arg0) -> tolerance = arg0);
    builder.addDoubleProperty("Acceleration", () -> acceleration,
            (arg0) -> acceleration = arg0);
    builder.addBooleanProperty("Enabled", () -> enabled,
            (arg0) -> enabled = arg0);
}
};
