...
public class Drive
{
	...
	/**
	 * Gets the averages of certain wheel groups. If ALL is selected, then the absolute value is 
	 * run to avoid issues where the average cancels them out.
	 * 
	 * @param encoderGroup Which wheel / set to find the average of. only LEFT, RIGHT and ALL are accepted.
	...
	public int getEncoderTicks(MotorPosition encoder)
	{
		...
		case LEFT:
		case LEFT_REAR:
			return encoders[0].getTicks();
		case RIGHT:
		case RIGHT_REAR:
			return encoders[1].getTicks();
		case LEFT_FRONT:
			if (encoders.length > 2)
				return encoders[2].getTicks();
		case RIGHT_FRONT:
			if (encoders.length > 3)
				return encoders[3].getTicks();
	...
	public boolean driveStraightInches(double distance, double speed, boolean accelerate)
	{
		...
		if (gyro != null)
			this.driveStraightGyro(speed, accelerate);
		else
	...
	public boolean accelerateTo(double leftSpeed, double rightSpeed, double time)
	{
		...
		this.getTransmission().driveRaw(leftSpeed * inRange(accelMotorPower), rightSpeed * inRange(accelMotorPower));
	 ...
	 */
	public void driveStraightGyro(double speed, boolean acceleration)
	{
	 ...
	 */
	public boolean pivotTurnDegrees(int degrees, double power, boolean useGyro)
	{
		...
		{
			if(degrees > 0 && Math.abs(getEncoderDistanceAverage(MotorPosition.LEFT)) > degreesToEncoderInches(degrees, true))
			{
				this.getTransmission().stop();
				pivotTurnDegreesInit = true;
				return true;
		...
		// Turning clockwise
		if (degrees > 0)
			this.getTransmission().driveRaw(power, -pivotDegreesStationaryPercentage);
		// Turning counter-clockwise
		else
			this.getTransmission().driveRaw(-pivotDegreesStationaryPercentage, power);
	...
	public boolean turnDegrees2Stage(int degrees, double power)
	{
		...
		{
			this.getTransmission().driveRaw(Math.signum(degrees) * turnDegrees2ndStagePower,
					-Math.signum(degrees) * turnDegrees2ndStagePower);
		}
		// 1st stage run (power)
		else
		{
			this.getTransmission().driveRaw(Math.signum(degrees) * power, -Math.signum(degrees) * power);
		}
