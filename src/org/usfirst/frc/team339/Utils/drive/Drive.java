...
public boolean accelerateTo(double leftSpeed, double rightSpeed, double percentPerSecond)
{
	...
	// multiply the acceleration we want by the seconds elapsed. After that, limit
	// the output between the minimum and maximum speeds for each side.

	leftOut = inRange(timeDelta * percentPerSecond * Math.signum(leftSpeed), -Math.abs(leftSpeed), Math.abs(leftSpeed));
...
public boolean arc(double speed, double radius, double arcLength, double accelerationTime)
{
	...
	// Compares the ratios of the expected rates vs the current ratio of rates from
...
public double getEncoderRate(MotorPosition encoderGroup)
{
	...
	// ===========================================================================
	case LEFT_FRONT:
		// If a four wheel system, get the left front. If not, the switch will move down
		// the line and get the left encoder.
		if (encoders.length == 4)
			return encoders[2].getRate();
	case LEFT:
		// If a four wheel system, average the two left. If not, then get the left
		// encoder.
		if (encoders.length == 4)
	...
	// ===========================================================================
	case ALL:
		// If a four wheel, then get the average of all 4. If not, then get the average
		// of the two.
		if (encoders.length == 4)
 ...
 * @param power        How fast the robot should be turning, in percentage (0.0
 *                     to 1.0)
 * @param accelerationTime Over how many seconds the motors should spool up, to
 *                     preserve accuracy during the turn.
 * @param usingGyro    If true, then the method will rely on the Gyro as it's
...
public boolean turnDegrees(int degrees, double speed, double acceleration, boolean usingGyro)
{
	...
	if (usingGyro && Math.abs(this.gyro.getAngle()) > Math.abs(degrees) - gyroFudgeFactor)
	{
		...
		turnDegreesInit = true;
		return true;
	} else if (!usingGyro && this.getEncoderDistanceAverage(MotorPosition.ALL) > degreesToEncoderInches(degrees, false))
	{
		this.transmission.stop();
		turnDegreesInit = true;
		return true;
