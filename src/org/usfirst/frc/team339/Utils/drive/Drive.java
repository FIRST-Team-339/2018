 ...
 */
public boolean accelerateTo_old (double leftSpeed, double rightSpeed,
        double time)
{
    ...
    this.transmission.driveRaw(leftSpeed * inRange(accelMotorPower),
        ...
        double percentPerSecond)
{
    ...
    // velocity.
    leftOut = Math.max(
            Math.min(Math.signum(leftSpeed) * timeSinceLastAccelReset
                    * percentPerSecond, Math.abs(leftSpeed)),
            -Math.abs(leftSpeed));
    rightOut = Math.max(
        ...
        double acceleration)
{
    ...
    this.accelerateTo_old(speed * leftSide, speed * rightSide,
            acceleration);
 ...
 * If mecanum transmission is being used (WHICH IT SHOULD NOT! USE
        ...
        boolean usingGyro)
{
    ...
    // Turning counter-clockwise
        ...
        this.transmission.driveRaw(-pivotDegreesStationaryPercentage,
                power);
        ...
        double acceleration, boolean usingGyro)
{
    ...
    else if (this.getEncoderDistanceAverage(
            MotorPosition.ALL) > degreesToEncoderInches(degrees, false))
        {
        this.transmission.stop();
        turnDegreesInit = true;
        return true;
