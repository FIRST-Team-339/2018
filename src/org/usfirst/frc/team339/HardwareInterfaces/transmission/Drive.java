...
public class Drive
{
	 ...
	 *            A sensor that uses a spinning disk to measure rotation.
	 */
	public Drive(TransmissionBase transmission, Encoder leftEncoder, Encoder rightEncoder, Gyro gyro)
	{
		this.transmissionType = transmission.getType();
	...
	/**
	 * Drives the robot in a straight line, correcting based on values gotten from encoders.
	 * 
	 * 
	 * @param speed
	...
	public void driveStraight(double speed, boolean acceleration)
	{
		...
		if (System.currentTimeMillis() - driveStraightLastTime > INIT_TIMEOUT)
		{
			...
			this.prevEncoderValues[1] = getEncoderTicks(MotorPosition.RIGHT);
		}

		// Left change since start minus right change since start
		int delta = (getEncoderTicks(MotorPosition.LEFT) - prevEncoderValues[0])
				- (getEncoderTicks(MotorPosition.RIGHT) - prevEncoderValues[1]);
		...
		// right.
		double leftSpeed = speed - (Math.signum(delta) * driveStraightConstant);
		double rightSpeed = speed + (Math.signum(delta) * driveStraightConstant);
	 ...
	 * 
	 * @deprecated Method relies on change between cycles on the rio, not absolute positioning. Use the improved driveStraight instead.
	 * 
	 * @param speed
