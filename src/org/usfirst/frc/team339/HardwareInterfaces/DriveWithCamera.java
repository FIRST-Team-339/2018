...
public class DriveWithCamera extends Drive
{
	 ...
	 */
	public DriveWithCamera(TransmissionBase transmission, Encoder leftFrontEncoder, Encoder rightFrontEncoder,
			Encoder leftRearEncoder, Encoder rightRearEncoder, GyroBase gyro, VisionProcessor visionProcessor)
	{
	 ...
	 * 
	 */
	public DriveWithCamera(TransmissionBase transmission, Encoder leftEncoder, Encoder rightEncoder,
			UltraSonic frontUltrasonic, UltraSonic rearUltrasonic, GyroBase gyro, VisionProcessor visionProcessor)
	{
	...
	public boolean driveToSwitch(double speed)
	{
		...
		case DRIVE_WITH_CAMERA:
			...
			if (centerX >= SWITCH_CAMERA_CENTER)
			{
				...
				this.getTransmission().drive(speed + DRIVE_CORRECTION, speed - DRIVE_CORRECTION);
			}
			...
			if (this.frontUltrasonic.getDistanceFromNearestBumper() <= CAMERA_NO_LONGER_WORKS)
		...
		case DRIVE_WITH_US:
			...
			if (this.frontUltrasonic.getDistanceFromNearestBumper() <= DISTANCE_FROM_WALL_TO_STOP)
	...
	public void visionTest(double compensationFactor, double speed)
	{
		...
		if (this.getCameraCenterValue() >= SWITCH_CAMERA_CENTER - CAMERA_DEADBAND
				&& this.getCameraCenterValue() <= SWITCH_CAMERA_CENTER + CAMERA_DEADBAND)
		{
			// driveStraight(speed, false);
			// System.out.println("We are aligned in the center");
		} else if (this.getCameraCenterValue() > SWITCH_CAMERA_CENTER + CAMERA_DEADBAND)
		{
	...
	public double getCameraCenterValue()
	{
		...
		{
			center = (visionProcessor.getNthSizeBlob(0).center.x + visionProcessor.getNthSizeBlob(1).center.x) / 2;
